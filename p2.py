...
thisAddr = "192.168.1.1"


if args.wifi:
  addrs = netifaces.ifaddresses('wlan0')
  wlanAddr = addrs[netifaces.AF_INET]
  thisAddr= wlanAddr[0]['addr']
else:
  eth0_addrs = netifaces.ifaddresses('eth0')
  eth0Addr = eth0_addrs[netifaces.AF_INET]
  thisAddr = eth0Addr[0]['addr']

print("Using " + thisAddr + "\n")
...
def upgradeTelnet():
    ...
    print 'Connecting ...'
    if args.wifi:
       tn = telnetlib.Telnet('192.168.10.1')
    else:
    ...
    print tn.read_until(prompt)
    tn.write("wget http://" + thisAddr +  "/chaos_calmer/15.05/bin/ar71xx/openwrt-ar71xx-generic-carambola2-squashfs-sysupgrade.bin -P /tmp\r\n")
    print tn.read_until(prompt)
    tn.write('ls -l /tmp\r\n')
    print tn.read_until(prompt)
    tn.write('uptime\n')
...
def upgrade():
	...
	if args.wifi:
        ...
        stdin, stdout, stderr = client.exec_command('wget http://" + thisAddr +  "/chaos_calmer/15.05/bin/ar71xx/openwrt-ramips-rt305x-carambola-squashfs-sysupgrade.bin -P /tmp')
        print stdout.read()
