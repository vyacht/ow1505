...
--    option enabled '1'
  ...
  if file_exists("/etc/config/vymodule") then
    ...
    -- add missing config of port 3 and 4
    if p3 == 0 then
      local itf = {
        port = 3, speed = 4800, type = "nmea0183", enabled = 1, device = "vyspi:///dev/ttyATH0" } 
      table.insert(hw.interfaces, itf) 
      local secname = "port" .. itf.port
      _uci_real:set("gpsd", secname, "interface")
      _uci_real:set("gpsd", secname, "port", itf.port)
      _uci_real:set("gpsd", secname, "type", itf.type)
      _uci_real:set("gpsd", secname, "speed", itf.speed)
      _uci_real:set("gpsd", secname, "enabled", itf.enabled)
      _uci_real:set("gpsd", secname, "device", itf.device)
      _uci_real:commit("gpsd")
    end
    if p4 == 0 then
      local itf = {
        port = 4, speed = 4800, type = "nmea0183", enabled = 1, device = "vyspi:///dev/ttyATH0" } 
      table.insert(hw.interfaces, itf) 
      local secname = "port" .. itf.port
      _uci_real:set("gpsd", secname, "interface")
      _uci_real:set("gpsd", secname, "port", itf.port)
...
function uploadFile(env)
  ...
  vWrite(string.format("{%q: %q}", "success", versionToString(file.software.version)))

  sys.exec(string.format("sleep 3; killall dropbear uhttpd; sleep 1; /sbin/sysupgrade %q", filename))
...
function setStaticNetwork(netName, device, ipaddr, prefix)
  ...
  os.execute("logger -s -t device -p daemon.info " .. "set static network for net= " .. netName .. ", device= " .. device .. ", ip= " .. ipaddr .. ", prefix= " .. prefix)
...
function resetSystem(params) 
 ...
 --
  ...
  local devs = {
    eth00 = {name = ETH_1, installed = 0},
    eth01 = {name = ETH_2, installed = 0},
    wifi  = {name = "radio0", installed = 0}
  }
  
  devs.eth00.installed = deviceInstalled(ETH_1)
  devs.eth01.installed = deviceInstalled(ETH_2)
  devs.wifi.installed = deviceInstalled("radio0")
  
  if devs.eth00.installed == 1 then
    setStaticNetwork("lan1", ETH_1, "192.168.1.1", 24)
    if devs.eth01.installed == 1 then
      setDhcpNetwork("wan", ETH_2)
    end
  else
    -- this case should actually never happen
    if devs.eth01.installed == 1 then
      setStaticNetwork("lan2", ETH_2, "192.168.1.1", 24)
    end
  end
...
function changeToLan(devices, addr, realDev, netName)

  if netName == "lan1" or netName == "lan2" then
    -- nothing to do
    return
  end
  ...
  local newNetName = ""
  if realDev == ETH_1 then
    newNetName = "lan1"
  end
  if realDev == ETH_2 then
    newNetName = "lan2"
  end
            
  -- check that we are not in range of the other lan
  if devices.eth01.installed and devices.eth00.installed then
    if realDev == ETH_1 then
      if IPInNetRange(ip, prefix, "lan2") then
        vWrite(string.format("{\"error\": \"New IP %s is in the IP range of the Ethernet 2\"}", ip))
        return  
      end
    end
    if realDev == ETH_2 then
  ...
  setStaticNetwork(newNetName, realDev, ip, prefix)
	
  _uci_real:commit("network")
  _uci_real:commit("dhcp")

  restartNetwork(netName, newNetName)

  vWrite("{}")

end

function changeToWan(devices, addr, realDev, netName)

  if netName ~= "lan1" and netName ~= "lan2" then
    vWrite(string.format("{\"error\": \"Change to WAN: Not a valid network (%s)!\"}", netName))
    return
  end
	
  if netName == "lan1" and devices.eth01.installed == 1 then
    vWrite(string.format("{\"error\": \"Change to WAN: only ethernet 2 can be changed to WAN!\"}"))
    return
  end

  -- check that WAN will not be enabled if there is no wifi or lan 
  -- preventing lock-out	
  if realDev == ETH_1 and devices.eth01.installed == 0 then 
	
    networkStatus(devices.wifi);

    if devices.wifi.available == 0 then 
      vWrite("{\"error\": \"You would potentially lock yourself out. You can not switch to WAN with one ethernet installed and Wifi disabled!\"}")
      return
    end
  ...
  _uci_real:delete("network", netName)
  _uci_real:delete("dhcp", netName)
  ...
  restartNetwork(netName, "wan")

  vWrite("{}")

end
	
function restartNetwork(oldInterface, newInterface)

  -- requires restart of ifdown/up lan and dnsmasq
  sys.call(string.format("env -i /sbin/ifdown %s >/dev/null", oldInterface))
  sys.call(string.format("env -i /sbin/ifup %s >/dev/null", newInterface))
...
function changeEthernet(params)
	...
	os.execute("logger -s -t device -p daemon.info " .. "device= " .. device .. ", lanwan= " .. lanwan .. ", ip= " .. addr)
	...
	-- we only need the name of the wifi network for network status
        local devices = {
          eth00 = {name = ETH_1, installed = 0, network = ""}, 
          eth01 = {name = ETH_2, installed = 0, network = ""}, 
	...
	if realDev == ETH_2 and devices.eth01.installed == 0 then
        	vWrite("{\"error\": \"Ethernet 2 is not installed!\"}")
        	return
	end

	if realDev == ETH_1 and devices.eth00.installed == 0 then
        	vWrite("{\"error\": \"Ethernet 1 is not installed!\"}")
        	return
	end

	if lanwan == nil then 
        	vWrite(string.format("{\"error\": \"Don't recognize empty command!\"}"))
        	return
        end

        if lanwan ~= "lan" and lanwan ~= "wan" then
        	vWrite(string.format("{\"error\": \"Don't recognize command (%s)!\"}", lanwan))
        	return
	end
	...
	if netName == "wan" then
	  if lanwan == "lan" then
		...
		changeToLan(devices, addr, realDev, netName)
	  end
	  if lanwan == "wan" then
	    -- later we could change a static IP for WAN
          ...
          -- is currently lan
          if lanwan == "wan" then  
            changeToWan(devices, addr, realDev, netName) 
          else 
...
function changeEthernetAddress(addr, realDev, net_name)
	...
	restartNetwork(net_name, net_name)
...
function changeNMEA(params) 
	...
	local portno = -1
  	...
  	if (type ~= nil) and ((type ~= "seatalk") and (type ~= "nmea0183")) then
          vWrite(string.format("{%q: \"Tried to change to unknown type.\"}", "error"))
          return
	end
        
        -- port is port name number and portno is array position
        if port ~= 2 then
        if (type ~= nil) and (hw.interfaces[portno].type ~= type) then
          vWrite(string.format("{%q: \"Only type of port 2 can be changed.\"}", "error"))
       	  return
        end
        end
	...
	if (speed ~= nil) then
	  ...
	  if speed == tonumber(hw.interfaces[portno].speed) then
            ...
            if hw.interfaces[portno].type == type then
              vWrite("{}")
              return
            end
	    ...
	    _uci_real:set("gpsd", secname, "speed", speed)      
            if hw.interfaces[portno].type == "nmea0183" then
              if type == "seatalk" then
                 _uci_real:set("gpsd", secname, "type", "seatalk")      
              end
            end
            if hw.interfaces[portno].type == "seatalk" then
              if type == "nmea0183" then
                 _uci_real:set("gpsd", secname, "type", "nmea0183")      
              end
            end
	    _uci_real:commit("gpsd")
...
function changeGps(params) 
	...
	elseif broadcast ~= nil then
          ...
          if not port then                               
                -- ipaddr may be empty
        	vWrite("{\"error\": \"Not a valid feed string! You need to specify a port. Only \'*:<port no>\' syntax is currently supported.\"}")
                return
          end
	  ...
	  if sname == "" then
	    sname = _uci_real:set("gpsd", "udp1", "interface")
	  end
